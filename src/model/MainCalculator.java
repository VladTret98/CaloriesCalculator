...
public class MainCalculator implements Serializable {
	...
	public void setDate(){
		if(this.activeUser.getActiveDay().getDate().getDate() == new Date().getDate()){
			return;
		}
		else {
			this.getActiveUser().setActiveDay(new Day());
		}
	}
	...
	public void addNewUser(String name, double weight, int goal, double lifeStyle) {
		for (Iterator<User> iterator = users.iterator(); iterator.hasNext();) {
			...
			if(name.equals(user.getName())) {
				JOptionPane.showMessageDialog(new JOptionPane(), "Имя занято");
				return;
			}
		}
